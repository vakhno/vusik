...
import { NextResponse } from "next/server";
import getProfileArticlesFilters, {
	ErrorResponse,
	SuccessResponse,
} from "@/features/article/loadProfileArticlesFilters/api/getProfileArticlesFilters";

export async function GET(req: Request): Promise<NextResponse<SuccessResponse | ErrorResponse>> {
	const { searchParams: URLSearchParams } = new URL(req.url);
	const result = await getProfileArticlesFilters({ searchParams: URLSearchParams });
	return result;
}
