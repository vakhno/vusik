...
import { AnimalProfileSearchSchemaType } from "@/features/animal/searchProfileAnimal/model/type";
...
import { queryGetProfileAnimalsFilter } from "@/entities/animal/model/query/animalsAllByPageProfileFilters";
...
const Index = ({ className = "", searchParams, userId, filterChage }: Props) => {
	const t = useTranslations();
	const { data: dataOptions } = queryGetProfileAnimalsFilter({
		searchParams: searchParams,
		id: userId,
	});
	// list of filter full available options
	const [animalFiltersOptionList, setAnimalFiltersOptionList] = useState<
		Partial<Record<keyof AnimalProfileSearchSchemaType, Option[]>>
	>({
		species: [],
		breed: [],
		sex: [],
	...
	const animalSearchSchema = AnimalProfileSearchSchema();
	...
	useMemo(() => {
		...
		const filterOptionsList = {
			...
			state: [],
			shelterId: [],
		} as Partial<Record<keyof AnimalProfileSearchSchemaType, Option[]>>;

		const { availableOptions, selectedOptions } = dataOptions as {
			availableOptions: Record<keyof AnimalProfileSearchSchemaType, string[] | boolean[] | ShelterType[]>;
			selectedOptions: Record<keyof AnimalProfileSearchSchemaType, string[] | boolean[]>;
		};
	...
	useEffect(() => {
		console.log("animalFiltersDefaultOptions", animalFiltersDefaultOptions);
		form.reset(animalFiltersDefaultOptions);
	}, [animalFiltersDefaultOptions]);

	const handleFilterChange = async (values: AnimalProfileSearchSchemaType) => {
	...
	const onSubmit = async (values: AnimalProfileSearchSchemaType) => {
